import pygame
import sys
from config import SCREEN_WIDTH, SCREEN_HEIGHT, FPS, COLOR_BACKGROUND, LEVELS, DYNAMIC_TOPOLOGIES
...
pygame.display.set_caption("Network Defender - AI/ML Edition")
...
GAME_STATE_FAILED = "failed"
game_flow_state = GAME_STATE_INTRO

def init_level(level_num):
    global game_state, tower_manager, enemies, game_flow_state

    # Provide override topology for endless level
    override_topo = None
    ...
    # Pass override topology at GameState creation
    game_state = GameState(level_num, override_topology=override_topo)
    tower_manager = TowerManager()
    enemies = []
    game_flow_state = GAME_STATE_INTRO

    print(f"DEBUG: Level {level_num} initialized with topology nodes: {game_state.nodes}")
...
def spawn_enemy():
    ...
    if game_state.level_number < 5:
        ...
        available_types = [t for t in game_state.level['enemy_types'] if t != 'LEGITIMATE']
        if game_state.level.get('legitimate_ratio', 0) > 0:
            import random
            if random.random() < game_state.level['legitimate_ratio']:
...
def update_game():
    ...
    for enemy in enemies[:]:
        ...
        if enemy.health <= 0 and not enemy.is_legitimate:
            ...
            reward = game_state.enemy_destroyed(enemy)
            ai_system.record_enemy_outcome(enemy.type, [], True)
        elif enemy.reached_goal:
            enemies.remove(enemy)
            game_state.enemy_reached_goal(enemy)
            ai_system.record_enemy_outcome(enemy.type, [], False)
    if game_state.should_spawn_enemy():
        spawn_enemy()
    # --- Topology switch logic ---
...
def draw_game():
    ...
    if game_state is None:
        ...
        return
    ui.draw_network(game_state.nodes, game_state.edges, game_state.start_node, game_state.goal_node)
    if tower_manager:
...
def main():
    global game_flow_state, current_level
