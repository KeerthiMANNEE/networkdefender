...
class UI:
    ...
    def update(self):
        # Call this each frame to clear popup after duration
        if self.popup_end_time:
            if time.time() > self.popup_end_time:
                self.popup_end_time = None
                self.popup_message = None
    ...
    def draw_hud(self, game_state):
        ...
        # Draw other info: money, core health, score, packets, success rate...
        ...
        if game_state.level.get("mode") != "ENDLESS":
            self.screen.blit(self.font_small.render(f"📦 {game_state.packets_spawned}/{game_state.level['total_packets']}", True, (200,200,200)), (410,45))
        else:
    ...
    def draw_level_intro(self, game_state):
        ...
        if 'mode' in game_state.level and game_state.level['mode']!='ENDLESS':
            self.screen.blit(self.font_small.render(f"Objective: Block {game_state.level['success_threshold']}% of {game_state.level['total_packets']} packets", True, (255,255,200)), (center_x-150, 360))
        else:
